...
fetch("https://docs.google.com/spreadsheets/d/1qKlWKpDkOpU1ZF6V6xGfutDY2NvcA8MNPnsv6GBkKPQ/gviz/tq?tqx=out:csv&sheet=GDDL").then(r => r.text()).then(csv => {
    ...
    fs.writeFileSync(path.resolve(__dirname, "v2", "daily.json"), newLineForEveryEntryButDontPrettifyEverything(daily));
    fs.writeFileSync(path.resolve(__dirname, "v2", "weekly.json"), newLineForEveryEntryButDontPrettifyEverything(weekly));
    fs.writeFileSync(path.resolve(__dirname, "v2", "event.json"), newLineForEveryEntryButDontPrettifyEverything(event));
});
